...
(defn selection-cfg [{:keys [name key source page order options help min sequential? quantity? collapsible? ui-fn new-item-text new-item-fn prereq-fn simple?] :as cfg}]
...
(defn option-cfg [{:keys [name key help selections modifiers prereqs order ui-fn] :as cfg}]
  {::name name
   ::key (or key (common/name-to-kw name))
   ::help help
   ::order order
